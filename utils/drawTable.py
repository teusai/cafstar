...
class DrawTable():
    def __init__(self, width, height, ratio, scores, gameTime, category, image=None, *args, **kwargs):
        ...
        self.remaining = pyglet.text.Label("Remaining:", color=fontColor, font_name=font, font_size=mainFontSize, x=center_x, y=center_y+(radius*0.75), anchor_x='center', anchor_y='center', batch=self.sBatch)
    ...
    def drawScoreboard(self, width, height, ratio, scores, gameTime, category, image=None):
        ...
        if int(gameTime) != int(self.currentTime):
            self.tBatch = pyglet.graphics.Batch()
            ...
            self.catchBorder = pyglet.shapes.BorderedRectangle(x=center_x-(boxWidth//2), y=center_y-(boxHeight//2), width=boxWidth, height=boxHeight, border=6, color=category["Color"], border_color=borderColor, batch=self.tBatch)
            self.catchLabel = pyglet.text.Label(category["Name"], color=(255, 255, 255, 255), font_name=font, font_size=mainFontSize, x=center_x, y=center_y, anchor_x='center', anchor_y="center", batch=self.tBatch)
