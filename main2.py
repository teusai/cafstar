...
peopleImages = [pyglet.resource.image(url) for url in peopleImageURLS[:200]]
...
def gameReset(dt):
    ...
    #DrawTable = drawTable.DrawTable()


def gameLoop(dt):
    # window.clear()
    global remainingTime, currentScore
    global dtTotal, dtCount
...
def gameEnd(dt):
    ...
    global dtTotal, dtCount
    print(dtCount, dtTotal, dtCount / dtTotal, dtTotal / dtCount)
    dtTotal = 0
    dtCount = 0
...
def spawnPerson(dt):
    ...
    personImage = peopleImages[min(int(personInfo['ID']) - 1, 199)]
